...
contract Farechain is AccessControl {
  /*** EVENTS ***/
  ...
  modifier isOrganisation {
    require((addressToIndex[msg.sender] != 0) &&
      organisations[addressToIndex[msg.sender]].valid, "Farechain: Caller is not a valid organisation");
    _;
  }
  ...
  function withdraw(uint256 amount) public onlyOwner {
    require(amount <= address(this).balance, "Farechain: Insufficient funds");
  ...
  function addOrganisation(
    ...
    require(!organisations[addressToIndex[organisationAddress]].valid, "Farechain: Organisation is already registered");
  ...
  function removeOrganisation(address organisationAddress) public onlyOwner {
    require(organisations[addressToIndex[organisationAddress]].valid, "Farechain: Organisation address is not valid");
  ...
  function addBags(uint16 quantity) public isOrganisation {
    require(quantity != 0, "Farechain: Cannot add zero bags");
  ...
  function buy(address organisationAddress, uint16 quantity) public payable {
    require(organisations[addressToIndex[organisationAddress]].valid, "Farechain: Organisation address is not valid");
    require(quantity != 0, "Farechain: Cannot purchase 0 bags");
    require(quantity <= organisations[addressToIndex[organisationAddress]].totalBags, "Farechain: Not enough bags in stock");
    require(msg.value >= (organisations[addressToIndex[organisationAddress]].bagPrice * quantity), "Farechain: Insufficient funds");
